...
# The dbplyr package 
#  XE database is run on Oracle 11g and thus one can not use dbplyr versions >2.0
#  Hence we need to install older versions
remotes::install_github("tidyverse/dbplyr@v1.4.4", force = TRUE)
# SIDEFFECTS: This means that one can not load the tidyverse package only its
#              child packages (dplyr, ggplot2, tidyr, tibble, ....)
#             It also means that if you want to access data via {mar} you
#              have to reinstall the latest version of {dbplyr}. Just do:
#              install.packages("dbplyr"} # overwrites the version 1.4.4
```
...
# SMB     30    73
# SMH     35    77, 78
# year = 2021 acts as a dummy here, since no 2022 data yet. Are kind of
#  pretending that we are still in the 2021 survey.
res <- xe::import_hafvog(id = 35, gid = c(77, 78), year = 2021)
# Here specify whatever cruise(s) you want to include in the overview
xe::munge_for_smxapp(res, cruise = c("TM1-2021", "A15-2021"))
```
