...
con <- mar::connect_mar()
#source("../../r/Pakkar2/xe/R/import_smx.R")
...
### Afli í staðaltogi árið 2018 (ok, ... 2017)


```{r}
renderLeaflet({
  
  x0 <- by.station %>% filter(ar == 2017) %>% select(lon, lat) %>% distinct()
  if(input$Type == "Numbers") {
    x <- 
      by.station %>% 
      filter(ar == 2017,
...
renderPlot({
  ...
  p <- 
    ...
    geom_ribbon(data = stadlar.lw %>% filter(tegund == as.numeric(input$Species)),
                ...
                fill = "pink") +
    geom_point(data = d %>% filter(ok.osl), aes(lengd, oslaegt), size = 1, alpha = 0.2, colour = "blue") +
...
renderPlot({
  ...
  p <- 
    ...
    geom_ribbon(data = stadlar.lw %>% filter(tegund == as.numeric(input$Species)),
                ...
                fill = "pink") +
    geom_point(data = d %>% filter(ok.sl), aes(lengd, slaegt), size = 1, alpha = 0.2, colour = "blue") +
...
renderPlot({
  ...
  p <- 
    ...
    geom_rect(data = stadlar,
                ...
                fill = "pink") +
    geom_point(data = d %>% filter(ok.sl.osl), aes(lengd, slaegt/oslaegt), size = 1, alpha = 0.2, colour = "blue") +
...
renderPlot({
  ...
  p <- 
    ...
    geom_rect(data = stadlar.tegundir %>% filter(tegund == as.numeric(input$Species)),
                ...
                fill = "pink") +
    geom_point(data = d %>% filter(ok.lifur.osl), aes(lengd, lifur/oslaegt), size = 1, alpha = 0.2, colour = "blue") +
...
renderPlot({
  ...
  p <- 
    ...
    geom_rect(data = stadlar.tegundir %>% filter(tegund == as.numeric(input$Species)),
                ...
                fill = "pink") +
    geom_point(data = d %>% filter(ok.kirtlar.osl), aes(lengd, kynfaeri/oslaegt), size = 1, alpha = 0.2, colour = "blue") +
  ...
  for(i in 1:length(years)) {
    p <-
      ...
      addPolylines(data = st.done.sp[st.done.sp$ar == years[i],],
                   group = as.character(st.done.sp$ar[i]))
  }
  p %>% 
